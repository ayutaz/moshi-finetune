...
V-real の話者A が 7.8 分である点は、**この規模のまま進めると決定した**。

> **訂正（2026-08-24）**: この節はもともと、7.8 分を「過去実験 189.9 分」に対する比で示し、
> 桁違いに少ないと書いていた。**その比較は誤りである。**
> 189.9 分は[実験計画](./j-moshi-tsukuyomi-ojousama-plan.md)の表の「話者原音」＝ Irodori-TTS の学習コーパスで、
> Moshi の学習データではない。Moshi に入った過去の量は同じ表の「合成対話 31.7 分」である。
> V-real の対話は 27.83 分、like-for-like で過去の **87.8%**。
> **「Moshi に与える量が過去より 1 桁以上少ない」という前提は、最初から成立していなかった。**
...
> **この代償の見立ても外れていた（2026-08-24）。** 実際の条件4 は陰性ではなく、
> 効果量基準（+0.02）を `v-tts/epoch3` が mean_delta_full_set **+0.0321** で満たしていた。
> 落ちたのは一貫性基準 8/10 だけで、その検出力は真の勝率 0.70 でも 0.383 しかない。
> 曖昧さを作ったのは**量ではなく判定基準の検出力**である。
> [M3 検証記録 §5.1〜5.2](./j-moshi-tsukuyomi-ojousama-m3-verification.md)。
...
- **条件4で差が出なかった場合、その結果は曖昧である。** train 分割は話者A の音声 8.67 分、
  M3 が使うのは72文で約7.8分。
  差が出ないことは「手法が効かない」ことも「検出力が足りない」ことも意味しうる。
  〔**2026-08-24 訂正**〕ここには当初、この量を過去実験の 189.9 分に対する比で示していたが、189.9 分は
  Irodori-TTS の学習コーパスであって Moshi の学習データではない。like-for-like は
  27.83 / 31.7 = **87.8%** であり、**量の不足はこの曖昧さの原因ではない。**
  実際に曖昧さを作ったのは 8/10 符号検定の検出力（真の勝率 0.70 で 0.383）だった。
...
**不合格。** 全6条件を評価し、V0/V1のいずれも採用基準を満たさなかった。判定と証拠は
[マイルストーン文書](./j-moshi-tsukuyomi-ojousama-milestones.md)にある。

計画自体は機能した。第1部の18ステップで13件の欠陥を課金前に潰し、GPU工程は両runとも完走、
生成550件を回収した。**失敗したのは実行ではなく仮説である** — この規模のデータで
この学習率・epoch数では、対象話者へ寄る前にモデルが崩壊する。

### やり直す場合の選択肢

| 案 | 内容 | 根拠 |
| --- | --- | --- |
| A | **epoch数を減らす**（1〜2 epoch） | V-tts epoch2 が劣化1/50 と最良。過学習の前に止める |
| B | **学習率を下げる**（`3e-5` → `1e-5`） | 過去実験は189.9分で`3e-5`。8.67分に同じ値は強すぎる可能性 |
| C | ~~**データを増やす**（寄せ集めコーパス申請）~~ → **撤回（2026-08-24）** | 根拠だった「8.67分は過去実験189.9分のごく一部」が比較対象の取り違えだった。189.9分はIrodori-TTSの学習コーパスで、Moshiの学習データは31.7分。like-for-likeは**87.8%**であり、量の不足は主因の候補ですらない |
| D | **更新範囲を狭める**（tempformer-only等） | 全パラメータ更新が8.67分に対して過剰かもしれない |

**AとBは追加GPU 1セッションで試せる**（データセットは構築済みで再利用できる）。

> **この 4 案は [M3-R 実行計画](./j-moshi-tsukuyomi-ojousama-m3r-plan.md) に置き換わった（2026-08-24）。**
> 案 C は根拠ごと撤回し、寄せ集めコーパスは申請しない。案 B の `1e-5` も fine-tuning の文献値
> （J-Moshi: tempformer 2e-6 / depformer 4e-6）の 2.5〜5 倍であり、M3-R は文献値を採る。
> そして 4 案のどれを選んでも、**判定に使う計器 3 件 — 基準線 control、崩壊検出器、条件4 の一貫性基準 —
> を先に直さなければ、再走の結果も同じ理由で読めない。** M3-R の第 0〜3 段（課金ゼロ）がそれである。

やり直す前に、**epoch 1〜2 を必ず export する**こと。今回は epoch 3・5 しか export せず、
最良の `v-tts/epoch2` を失った。

### この計画から学んだこと

- **打ち切り線は計算ではなくアラームである必要がある。** 経過時間ではなく見積もりを見続けた結果、
  14.0時間の線に対し25.21時間動き、上限を超過した
- **中間 epoch を捨ててはいけない。** 最良の腕が中間にあった
- **eval loss をW&Bだけに出す実装は、W&Bを使わない運用で全損する。** 修正済み
